...
export default function ServiceList(){
  ...
  React.useEffect(()=>{
    let alive = true
    apiIndex.request('/services').then(d=>{ if(alive) setItems(Array.isArray(d) ? d : []) }).catch(e=>setError(e.message)).finally(()=>setLoading(false))
    return ()=>{ alive=false }
  },[])
  async function onDelete(id){
    if(!confirm('Delete this item?')) return
    await apiIndex.request(`/services/${id}`, { method: 'DELETE' })
  ...
  return (
    <div style={{marginTop:12}}>
      ...
      <div style={{marginTop:8}}><button className="btn" onClick={async()=>{{await apiIndex.request('/services', { method: 'DELETE' }); setItems([]); toast('All deleted')}}}>Delete All</button></div>
    </div>
  )
}
