...
export default function ProjectForm(){
  ...
  const [form,setForm] = React.useState({ title:'', completion:'', description:'' })

  React.useEffect(()=>{
    if(!isEdit) return
    let alive = true
    api.get(RES.projects, id).then(d=>{{ if(alive) setForm(d) }}).catch(e=>setError(e.message)).finally(()=>setLoading(false))
    return ()=>{{ alive=false }}
  },[id])
  ...
  async function onSubmit(e){
    ...
    setSaving(true)
    try{
      if(isEdit) await api.update(RES.projects, id, form)
      else await api.create(RES.projects, form)
